...
for(const c of data) {
  ...
  content += `<div class="character-top" style="--character:${c.color}"><div>${quote}<div class="summary-copy">${md(c.intro)}</div><div class="tag-row">${badge(c.supporting?'관련 문서 기반 정리':'원문 반영','pink')}${badge(c.profile['신장']?'신장·체중 반영':'제작 수치 TBD')}${badge(c.supporting?'등장·언급 화만 수록':'가상 캐스팅 ≠ 확정 성우')}</div><p class="source-ref">출처: ${c.supporting?'관련 캐릭터 문서 · 본문에 대조 근거 표기':'제공 캐릭터 문서 · 개요 / 특징 / 프로필'}</p></div><aside class="infobox" aria-label="캐릭터 기본 정보"><div class="box-title">${escape(c.name)}</div>${artwork.portrait(c)}<dl class="kv">${mini.map(([k,v])=>`<dt>${k}</dt><dd>${escape(v||'TBD')}</dd>`).join('')}</dl></aside></div>`;
...
let characters = notice('수록 범위',`총 ${data.length}명의 원작 기준 문서입니다. 추가 조연 문서는 등장·언급이 확인되는 화만 정리했습니다. ‘협력자·조력자’는 페어리즈를 돕는 인물이며, 카즈키는 교단의 외부 협력자로 ‘흑성교단’에 분류합니다. 소속 필터는 문서 탐색용이며, 소속 변화·생존 여부는 캐릭터별 본문을 기준으로 확인합니다.`);
...
story+=section('chapters','2','화별 목차',`<p class="small muted">인물 이름을 누르면 해당 화 기록으로 이동합니다. 추가 조연 문서는 등장·언급이 확인된 기록만 연결합니다. 기존 주요 인물 문서는 미등장 설명을 포함하므로 실제 등장 여부는 각 기록에서 확인합니다.</p>${volumeButtons}${volumesMeta.map(([title],i)=>{
