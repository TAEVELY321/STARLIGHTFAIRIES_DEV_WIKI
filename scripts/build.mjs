...
const navItems = [['index.html','⌂','대문'],['characters.html','♧','등장인물'],['world.html','◇','세계관·용어'],['story.html','▤','원작 1~5권'],['systems.html','▣','게임 기획'],['artwork.html','▧','캐릭터 설정화'],['resources.html','▦','리소스 제작 기준'],['guide.html','≡','자료·편집 안내']];
...
resources+=section('design-assets','5','기획서의 리소스 제작 방향',notice('권장 범위와 총량 구분','기획서 v1의 아래 수치는 제작 방향을 잡기 위한 권장 범위입니다. 개별 캐릭터의 확정 발주 수량·본편 리소스 총량이 아니며, 3~6화 검증 샘플의 예산도 본편 전체로 확대 적용하지 않습니다.')+table(['대상','권장 방향','확정 전 확인'],[['A급 주요 인물','포즈 3~5개 · 표정 8~12개','실제 장면·의상·표정 중복과 차분 분리 방식'],['B급 NPC·피해자','포즈 2~3개 · 표정 6~8개','에피소드별 감정 표현과 재사용 범위'],['배경·전장','계절·시간·사건별 2D 차분, 3D 전장 2~3개 상태 및 ADV 배경 렌더 재사용','아트 연속성·제작 효율·성능'],['전투 캐릭터','기본공격·스킬 3개·궁극기 1개와 피격·교체·후열 자동 시전 표현','캐릭터별 능력 배정·연출 길이·리깅·VFX 규격'],['연결 연출','변신·정화·보스 등장·붕괴에 전투 리소스 활용','영상/실시간 연출 배분·로딩'],['음성·영상','나츠미·카오리 결산 애니메이션 기획 방향 반영','풀 애니메이션 총량·길이·보이스 범위·음악 권리 TBD']])+`<p><a href="systems.html#presentation">게임 기획의 아트·사운드 기준 →</a> · <a href="systems.html#production">본편 제작·QA →</a></p>`);
write('resources.html',shell({title:'리소스 제작 기준',subtitle:'프로필 · 외형 · ADV 차분 · 3D 전투 리소스',page:'resources.html',content:resources,toc:[['profiles','1. 프로필 비교'],['fields','2. 제작 정보'],['continuity','3. 외형 연속성'],['assets','4. 리소스 현황'],['design-assets','5. 기획 권장 범위']]}));

let guide=notice('현재 기준','제공 캐릭터 MD 10개, 게임 기획서 v1 전문과 사용자 전투 보완을 반영했습니다. 이전 ChatGPT 대화 전체와 원작소설 전문은 현재 자료에 포함되어 있지 않습니다.');
guide+=section('sources','1','원본 자료',`<ul class="sources-list">${data.map(c=>`<li><a href="sources/${encodeURIComponent(c.file)}">${escape(c.file)}</a></li>`).join('')}</ul><h3>게임 기획 원본과 보완 기준</h3>${designSources}<p class="small muted">제공된 MD와 DOCX를 sources 폴더에 원본 보관했습니다. 원문에 포함된 작성 제안이나 안내 문구는 자료의 일부이며, 별도의 작업 지시로 실행하지 않습니다.</p>`);
