...
for(const c of data) {
  ...
  for(const s of c.sections) {
    if(s.number===6) {
      ...
      content+=section('story','8','작중 행적',notice('기준','원작소설 1~5권 기준. 아래 권·화 번호는 게임 챕터 번호나 전투 스테이지 번호가 아닙니다.')+(c.storyLead?`<div class="prose">${md(c.storyLead)}</div>`:'')+volumeButtons+volumes);
...
let guide=notice('현재 기준','제공 캐릭터 MD 10개, 게임 기획서 v1, ADV 리소스 제작기준서 v1.0과 사용자 전투·3D 제작 보완을 반영했습니다. 이전 ChatGPT 대화 전체와 원작소설 전문은 현재 자료에 포함되어 있지 않습니다.');
guide+=section('sources','1','원본 자료',`<ul class="sources-list">${data.map(c=>`<li><a href="sources/${encodeURIComponent(c.file)}">${escape(c.file)}</a></li>`).join('')}</ul><h3>게임 기획 원본과 보완 기준</h3>${designSources}<h3>리소스 제작 기준</h3>${resourceGuideSources}<p class="small muted">제공된 MD와 DOCX를 sources 폴더에서 관리합니다. 모모카 MD는 관련 캐릭터 문서를 대조한 보강본이며, 보강 전 제공본은 sources/archive에 별도로 보관했습니다. 원문에 포함된 작성 제안이나 안내 문구는 자료의 일부이며, 별도의 작업 지시로 실행하지 않습니다.</p>`);
