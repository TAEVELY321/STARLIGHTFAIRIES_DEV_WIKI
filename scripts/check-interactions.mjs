...
const threeTabs=Array.from({length:3},()=>new Element());
const threePanels=Array.from({length:3},(_,i)=>new Element({hidden:i!==0}));
const threeWidget=new Element();
threeWidget.querySelectorAll=selector=>selector==='[data-portrait-tab]'?threeTabs:threePanels;
...
doc.querySelectorAll=s=>({'.character-card':cards,'[data-group-filter]':filters,'[data-volumes]':[],'[data-portrait-switch]':[portraitWidget,threeWidget],details:[details,printTarget]}[s]||[]);
...
threeTabs[1].emit('click');
assert.deepEqual(threePanels.map(p=>p.hidden),[true,false,true],'Darkness form displays independently');
threeTabs[2].emit('click');
assert.deepEqual(threePanels.map(p=>p.hidden),[true,true,false],'Fairies form displays independently');
threeTabs[2].emit('keydown',{key:'ArrowRight',preventDefault(){}});
assert.deepEqual(threePanels.map(p=>p.hidden),[false,true,true],'Third form wraps to the first');
threeTabs[0].emit('keydown',{key:'ArrowLeft',preventDefault(){}});
assert.equal(threeTabs[2].attributes['aria-selected'],'true','Left arrow wraps to the third form');
threeTabs[2].emit('keydown',{key:'Home',preventDefault(){}});
assert.equal(threeTabs[0].attributes['aria-selected'],'true');
threeTabs[0].emit('keydown',{key:'End',preventDefault(){}});
assert.equal(threeTabs[2].attributes.tabindex,'0','End focuses the third form');
