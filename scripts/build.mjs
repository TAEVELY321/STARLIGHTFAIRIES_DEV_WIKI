...
for(const c of data) {
  ...
  const mini = [['이름',c.name],[c.supporting?'역할':'변신·이명',c.alias],['연령',c.profile['연령']],['신장',c.profile['신장']],['체중',c.profile['체중']],[c.supporting?'서사 역할':'상징',c.power],['구분',c.group==='협력자·조력자'?(c.id==='kazuki'?'흑성교단 외부 협력자':'기록자·조력자'):c.group],[c.supporting?'첫 확인':'첫 등장',c.profile['첫 확인']||c.profile['첫 등장']||c.profile['첫 언급']]];
