...
원작소설 DOCX 5권 · 직접 대조한 캐릭터 MD 16개를 관리합니다. 모든 캐릭터의 주요 행적은 실제 등장·회상·언급을 구분하고 소설의 권·화 근거를 기록합니다. 나기사는 1화, 치요·리코는 2화부터 수록합니다. 원작 목차와 대조한 대사·판본 해시는 sources/reference/novel-index.json, 정정 내역과 재확인 지점은 sources/reference/원작_대조_기록.md에서 확인합니다. 사용자 입력 신체정보·생일·테마곡·외전 메모는 별도 제작 설정으로 보존합니다. 원작 DOCX 전문은 웹에 복제하지 않습니다.
