...
const outputRoot = process.env.STARFAIR_OUTPUT_ROOT ? path.resolve(process.env.STARFAIR_OUTPUT_ROOT) : root;
const read = file => fs.readFileSync(path.join(root, file), 'utf8').replace(/^\uFEFF/, '').replace(/\r\n/g, '\n');
const write = (file, value) => { fs.mkdirSync(path.dirname(path.join(outputRoot, file)), {recursive:true}); fs.writeFileSync(path.join(outputRoot, file), value, 'utf8'); };
...
const sourceCounts = `원작소설 DOCX 5권 · 직접 대조한 캐릭터 MD ${data.length}개`;
...
// Primary-source notes stay visible beside each selected appearance or reference.
const storyMarkdown = (c, value) => md(value).replace(/<p>근거: ([^<]+)<\/p>/g, '<p class="source-ref">근거: $1 <a href="../guide.html#novel-audit">판본·대조 기록</a></p>');
const bodyValue = (c, key, unit='') => c.profile[key] ? escape(c.profile[key]) : `${TBD}${unit?` <span class="muted">${unit}</span>`:''}`;
...
for(const c of data) {
  ...
  const mini = [['이름',c.name],[c.supporting?'역할':'변신·이명',c.alias],['연령',c.profile['연령']],['신장',c.profile['신장']],['체중',c.profile['체중']],[c.supporting?'서사 역할':'상징',c.power],['구분',c.group],[c.supporting?'첫 확인':'첫 등장',c.profile['첫 확인']||c.profile['첫 등장']||c.profile['첫 언급']]];
  let content = notice('스포일러 안내','원작소설 1~5권의 정체·결말을 포함합니다. 작중 행적은 원작 기준이며, 게임화 과정에서 일부 내용이 변경·축소·확장될 수 있습니다.');
  content += `<div class="character-top" style="--character:${c.color}"><div>${quote}<div class="summary-copy">${md(c.intro)}</div><div class="tag-row">${badge('원작소설 직접 대조','pink')}${badge(c.profile['신장']?'신장·체중 반영':'제작 수치 TBD')}${badge(c.supporting?'등장·언급 화만 수록':'가상 캐스팅 ≠ 확정 성우')}</div><p class="source-ref">출처: 원작소설 1~5권 · 권·화 근거는 본문 참조</p></div><aside class="infobox" aria-label="캐릭터 기본 정보"><div class="box-title">${escape(c.name)}</div>${artwork.portrait(c)}<dl class="kv">${mini.map(([k,v])=>`<dt>${k}</dt><dd>${escape(v||'TBD')}</dd>`).join('')}</dl></aside></div>`;
  ...
  content+=section('production','2','리소스 제작용 프로필',`<p class="muted small">신체 수치는 기본 프로필의 사용자 제공 제작 설정을 따르며, 기재하지 않은 항목은 추정하지 않습니다. 성격·모에요소는 원문 묘사를 바탕으로 묶은 제작용 분류이며, 추가 공식 설정을 의미하지 않습니다.</p>${profileTable(resourceRows)}<h3>형태·의상 구분</h3><p class="muted small">${c.supporting?'자료에서 확인된 형태와 별도 표기한 ADV 제작안을 정리했습니다.':'원작에 등장하는 형태를 정리했습니다.'} 게임에서 필요한 모델·스탠딩·차분의 범위는 TBD입니다.</p>${forms}${notice('디자인 메모',escape(c.note))}${profileTable([['ADV','공통 규격·추천 수량: <a href="../adv-guide.html#standing">ADV·2D 제작 가이드</a>. 캐릭터별 발주·완료 상태: '+TBD],['3D 전투','<a href="../modeling-guide.html">파츠 분리·전체 바디・Shape Key 가이드</a>. 리깅・VFX・소켓・성능 예산: '+TBD]])}`);
  ...
  const sourceDescription = '사용자가 제공한 원작소설 1~5권 DOCX의 주요 장면을 직접 대조했습니다. 다른 캐릭터 문서를 사실 확인의 근거로 대체하지 않습니다. 기본 프로필의 사용자 제작 수치·생일과 기존 테마곡·외전 메모는 별도 설정으로 보존합니다. <a href="../guide.html#novel-audit">기준 판본·정정 내용·재확인 지점 →</a>';
  ...
  if(c.id==='momoka') content+=notice('모모카 문서 보강 이력',`2026.09.06 · 원작소설 1~5권 직접 대조로 개요·특징·관계·행적·어록을 다시 정리했습니다. 기존 수치·생일·별도 제작 설정은 보존했습니다. <a href="../sources/archive/${encodeURIComponent('사쿠라이_모모카_페어리_로제트_위키_보강전_2026-09-06.md')}">보강 전 제공 MD 열기 →</a>`);
...
home+=section('fairies','2','등장인물',strip+table(['구분','인물','문서 안내'],[['스타라이트 페어리즈',data.filter(c=>c.group==='페어리즈').map(c=>link(c)).join(' · '),'사랑 · 희망 · 용기 · 믿음'],['흑성교단 관련 인물',data.filter(c=>c.group==='흑성교단').map(c=>link(c)).join(' · '),'교단 간부·외부 협력자 · 소속 변화와 결말은 각 문서 참조'],['협력자·조력자',data.filter(c=>c.group==='협력자·조력자').map(c=>link(c)).join(' · '),'페어리즈의 활동을 돕는 인물'],['마을·여학원',data.filter(c=>c.group==='마을·여학원').map(c=>link(c)).join(' · '),'일상·가족·학교 인물 · 등장·언급이 확인되는 행적']])+'<p class="small"><a href="characters.html">전체 캐릭터 문서와 검색 →</a></p>');
...
home+=section('status','4','자료 반영 현황',table(['항목','현황'],[['캐릭터 문서',`${sourceCounts} · 화별 행적 ${data.reduce((n,c)=>n+c.chapters.length,0)}건`],['제작용 프로필','신장·체중·쓰리사이즈·성격 키워드·모에요소·형태·소품 정리'],['원작 본문 대조','제공 DOCX 1~5권 직접 대조 · 주요 행적과 발화에 권·화 근거 표기'],['최신 게임 기획','기획서 v1(2026.08.29) + 사용자 전투 보완(2026.09.06) 반영. 전체 게임 기준 · 권장안·잠정 설계 구분'],['로고','제공된 회사 로고·게임 타이틀 로고 반영']])+`<p class="small muted">출처·편집 기준과 필요한 추가 자료는 <a href="guide.html">자료·편집 안내</a>에서 확인할 수 있습니다.</p>`);
write('index.html',shell({title:'스타라이트 페어리즈',subtitle:'캐릭터 · 원작 설정 · 게임 제작 자료',content:home}));

let characters = notice('수록 범위',`총 ${data.length}명의 원작 기준 문서입니다. 모든 캐릭터는 등장·회상·언급이 확인되는 주요 화만 정리했습니다. ‘협력자·조력자’는 페어리즈를 돕는 인물이며, 카즈키는 교단의 외부 협력자로 ‘흑성교단’에 분류합니다. 소속 필터는 문서 탐색용이며, 소속 변화·생존 여부는 캐릭터별 본문을 기준으로 확인합니다.`);
...
let story=notice('원작 기준','1~50화 목차는 제공 원작소설 DOCX의 표제에서 확인했습니다. 권별 구분명은 기존 위키의 명칭을 유지합니다. 게임 전체 설계는 기획서 v1과 사용자 전투 보완을 반영하며, 게임 1.0 수록 범위·실제 챕터 대응·전투 배치·장면별 변경은 TBD입니다.');
story+=section('volume-index','1','권별 개요',table(['권','제목','주요 흐름'],volumesMeta.map(([title,text],i)=>[`<a href="#volume-${i+1}">${i+1}권</a>`,title,text])));
story+=section('chapters','2','화별 목차',`<p class="small muted">인물 이름을 누르면 해당 화 기록으로 이동합니다. 모든 인물은 실제 등장·회상·명시적 언급이 있는 주요 기록만 연결합니다. 기록의 구분을 확인해 주세요. 이름 언급 전체를 나열한 전수 출연표는 아닙니다.</p>${volumeButtons}${volumesMeta.map(([title],i)=>{
  const chapters=novelIndex.chapters.filter(c=>c.volume===i+1);
...
let resources=productionGuides.links+notice('제작 기준','원문 설정과 제작 규격을 분리해 관리합니다. 신체정보는 2026.09.06 사용자 제공 제작 설정을 반영했습니다. 기재하지 않은 수치·외형 디테일은 TBD로 남기며, 원작 묘사를 임의 수치로 환산하지 않습니다.');
resources+=section('profiles','1','캐릭터 프로필 비교',table(['캐릭터','연령 (원문)','신장','체중','B(컵) / W / H','제작용 프로필'],data.map(c=>[link(c),escape(c.profile['연령']),bodyValue(c,'신장','cm'),bodyValue(c,'체중','kg'),bodyValue(c,'쓰리사이즈 (B / W / H)','cm'),`<a href="characters/${c.id}.html#production">외형·성격·형태 →</a>`])));
...
let guide=notice('현재 기준',`${sourceCounts}, 게임 기획서 v1, ADV 리소스 제작기준서 v1.0과 사용자 전투·3D 제작 보완을 반영했습니다. 원작 행적은 제공된 1~5권 DOCX를 직접 대조했습니다. 이전 ChatGPT 대화 전체가 자동으로 포함되는 것은 아닙니다.`);
guide+=section('sources','1','원본 자료',`<ul class="sources-list">${data.map(c=>`<li><a href="sources/${encodeURIComponent(c.file)}">${escape(c.file)}</a> ${badge('원작 직접 대조')}</li>`).join('')}</ul><h3>게임 기획 원본과 보완 기준</h3>${designSources}<h3>리소스 제작 기준</h3>${resourceGuideSources}<p class="small muted">캐릭터 MD와 제작 기준은 sources에서 관리합니다. 원작소설 DOCX 원본은 사용자가 제공한 위치에 두고 웹에는 전문을 복제하지 않았습니다. 문서 안의 작성 제안은 자료 내용으로 취급하며 작업 지시로 실행하지 않습니다.</p>`);
guide+=section('novel-audit','자료','원작소설 대조 기록',notice('2026.09.06 대조','16개 캐릭터 문서의 주요 장면과 발화를 원작 DOCX에서 확인했습니다. 신체정보·생일·테마곡·외전 메모는 사용자 제작 설정으로 보존합니다. 회상과 환영은 현재의 실제 행동과 구분합니다.')+table(['기준 판본','확인 범위'],novelIndex.sources.map(s=>[escape(s.source),`${s.volume}권 · ${(s.volume-1)*10+1}~${s.volume*10}화`]))+`<p><a href="sources/reference/${encodeURIComponent('원작_대조_기록.md')}">주요 정정·보강과 재확인 지점 읽기 →</a></p><details><summary>대조 내용 펼치기</summary><div class="prose">${md(read('sources/reference/원작_대조_기록.md'))}</div></details><p class="small"><a href="sources/reference/novel-index.json">50화 표제·인물별 수록 화·대사 문단·판본 SHA-256</a></p>`);
guide+=section('rules','2','문서 작성 기준',table(['표기','의미'],[[badge('확정 기준','pink'),'사용자 명시 규칙 또는 기획서의 확정 방향. 출처를 함께 기록.'],[badge('권장안'),'기획서가 제안하는 방향. 확정 사양·본편 제작량이 아님.'],[badge('프로토타입 검증'),'구현·테스트 후 유지·변경을 결정할 잠정 설계.'],[badge('결정 필요','tbd'),'후보·방향은 있으나 최종 결정이 필요한 항목.'],[badge('원작소설 직접 대조','pink'),'제공 DOCX의 사건·발화를 직접 확인한 서술. 등장·회상·언급 및 별도 제작 설정을 구분.'],[badge('원문 기반 요약'),'흩어진 묘사를 제작용 키워드로 묶은 항목. 추가 공식 설정이 아님.'],[badge('제작용 분류'),'모에요소·형태 목록 등 작업을 위한 분류. 구현 범위·수량 확정이 아님.'],[TBD,'현재 제공 자료로 정할 수 없는 정보. 임의 추정 없이 추후 갱신.'],['미공개 / 해당 없음','원문 상태를 보존. 인간과 다른 존재의 나이·신체 규격 등은 적용 가능 여부부터 확인.']]));
guide+=section('missing','3','추가 확인이 필요한 자료',`<ul class="plain-list"><li><strong>미정 설계의 결정 결과:</strong> 시작 선봉·AP 회복·교체/후열 처리·속도 검증, 성장·음성·저장·기술·출시 범위. <a href="systems.html#pending">게임 기획의 결정 목록</a> 참조.</li><li><strong>후속 개정본·확장 설정:</strong> 판본이 바뀔 때의 변경 내용, 붉은 반지 소녀와 후속작의 연결, 원작 대조 기록의 재확인 지점.</li><li><strong>캐릭터 제작 설정:</strong> 아직 신장·체중이 기재되지 않은 인물의 수치, 등신·정확한 색상·소재·의상 규격.</li><li><strong>추가 캐릭터 이미지:</strong> 설정화 페이지의 TBD 형태·인물과 의복을 착용한 체형 참고 시트.</li><li><strong>게임화 변경표:</strong> 원작 화와 게임 장면의 대응, 축소·확장·변경 사유.</li></ul>`);
guide+=section('editing','4','편집·추가 방법',`<p>일반 열람은 <code>index.html</code>을 열면 됩니다. 생성된 HTML은 별도 서버 없이 동작하며, 검색·테마·모바일 메뉴에는 JavaScript를 사용합니다.</p>${table(['수정 대상','파일'],[['ADV·3D 제작 가이드','scripts/resource-guides.mjs'],['배경·포즈·표정·화별 CG 표','assets/resource-guide.json'],['게임 기획 내용','scripts/game-design.mjs'],['기획 원본·보완 기준','sources/design'],['원작 내용·신체정보','sources/캐릭터_위키.md (기본 프로필의 신체정보를 상세 페이지·비교표에 공통 반영)'],['제작용 요약·형태 목록','assets/characters.json'],['정면 표시 영역·설정화·팔레트','assets/artwork.json'],['시트 원본 / 브랜드 이미지','sources/CharacterSheet / sources/branding'],['페이지 구성·공통 문서','scripts/build.mjs'],['디자인','assets/style.css'],['검색·테마·메뉴','assets/app.js']])}<p>원본과 제작 정보를 수정한 뒤 Node.js 환경에서 <code>npm install</code>, <code>npm run build</code>, <code>npm run check</code> 순서로 실행하면 HTML과 검색 자료를 갱신할 수 있습니다. 재생성하면 생성된 HTML의 직접 수정 내용은 덮어씁니다.</p><p>위키 파일은 현재 내부 개발용 자료입니다. 표시 문구와 검색엔진 제외 설정은 접근 제어가 아니므로, 실제 공개 범위는 게시 환경에서 결정합니다.</p>`);
write('guide.html',shell({title:'자료·편집 안내',subtitle:'출처와 미확정 항목을 함께 관리합니다',page:'guide.html',content:guide,toc:[['sources','1. 원본 자료'],['novel-audit','원작소설 대조 기록'],['rules','2. 작성 기준'],['missing','3. 추가 자료'],['editing','4. 편집 방법']]}));
let artPage=notice('설정화 안내','형태를 펼쳐 시트 전체를 확인할 수 있습니다. 페어리즈는 변신 전·변신 후·바디, 교단의 절광체 인물은 인간 형태·절광체 형태·바디 순서입니다. 카즈키 등 별도 변신 구분이 없는 인물은 시트·바디 순서입니다.');
